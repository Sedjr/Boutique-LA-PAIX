...
import { Wallet, ArrowDownCircle, ArrowUpCircle, Calculator, Send } from 'lucide-react';
...
export const MyCash: React.FC<MyCashProps> = ({ userBoutique, userUid }) => {
  ...
  const handleCloture = () => {
    ...
    // Admin phone number could be dynamic, but for now we use a placeholder or the owner's known context if available
    // Since we don't have the admin phone, we open WhatsApp share or a generic link
    ...
    <div className="space-y-6">
      ...
      <div className="pt-6">
        <Button 
          ...
          className="w-full h-16 text-xl font-black gap-3 shadow-xl shadow-green-500/20 bg-green-600 hover:bg-green-700"
        >
          <Send className="h-6 w-6" />
          CLÔTURE DE JOURNÉE (WhatsApp)
        </Button>
        <p className="text-center text-xs text-muted-foreground mt-4 italic">
          Cette action envoie le bilan final de votre boutique à l'administrateur.
        </p>
      </div>
    </div>
  );
